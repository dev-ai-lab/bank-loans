package com.bank.loans.web.api.v1;
...
public class LoansController {
    ...
    public ResponseEntity<LoansDto> fetchLoanDetails(@RequestHeader("bank-correlation-id") String correlationId,
                                                     @RequestParam
                                                               @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits")
                                                               String mobileNumber) {
        logger.debug("tracing correction-id: {}", correlationId);
        logger.debug("fetchLoanDetails method start"); // open telemetry will add tag, tace-id and span-id
